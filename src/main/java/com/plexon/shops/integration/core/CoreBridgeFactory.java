...
public final class CoreBridgeFactory {
    ...
    private static final String CORE_BRIDGE_CLASS = "com.plexon.shops.integration.core.PlexonCoreBridge";
    ...
    public static CoreBridge resolve(JavaPlugin plugin) {
        ...
        if (corePlugin == null) {
            plugin.getLogger().info("PlexonCore not installed; starting PlexonShops in standalone compatibility mode.");
        ...
        if (!corePlugin.isEnabled()) {
            plugin.getLogger().warning("PlexonCore is installed but disabled; starting PlexonShops standalone.");
